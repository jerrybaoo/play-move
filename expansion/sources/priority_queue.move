...
// It fork from sui framework.

/// Priority queue implemented using a max heap.
module expansion::priority_queue {
    use std::vector;
    ...
    public fun entry_priority<T: drop>(entry: &Entry<T>) : u64 {
        entry.priority
    }

    public fun set_entry_priority<T: drop>(entry: &mut Entry<T>, priority: u64) {
